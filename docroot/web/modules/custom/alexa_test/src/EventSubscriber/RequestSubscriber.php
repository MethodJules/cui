...
namespace Drupal\alexa_test\EventSubscriber;
...
class RequestSubscriber implements EventSubscriberInterface {
  ...
  public function onRequest(AlexaEvent $event) {
    ...
    switch ($request->intentName) {
      ...
      case 'DrupalIntent':
        $response->respondSSML('<speak>The DrupalIntent works.</speak>');
        $response->shouldEndSession = TRUE;
        break;

      case 'MethodTest':
        
        $slot_name =  $request->getSlot('Methodenname');
        $slot_info = $request->getSlot('Info');

        // Checken, ob slot gesetzt wurde.
       /* if ($slot_name == FALSE && $request->data['request']['dialogState'] == "STARTED") {

          // Slot war leer, aber Intent wurde erkannt > Nachfragen.
          $response->respond('Zu welcher Methode möchtest du Informationen erhalten?')
            ->reprompt('Bitte sag zum Beispiel "Zählen" oder "Predict Next Year\'s Headline"');
        }
        else {

          // Slot war gefüllt und wurde erkannt, hole die passende Entity als Response.
          $response->respondSSML($this->getMethod($slot_name));
        ...
        if ($slot_info == FALSE) {
          ...
          $response->respondSSML($this->getMethodInfo($slot_name, $slot_info));
          $response->shouldEndSession = FALSE;
        }
      ...
      default:
        $response->respondSSML('<speak>Hallo, hier ist Drupal</speak>');
        $response->shouldEndSession = FALSE;
        break;
    }
  }
  ...
  public function getMethod($term = '') {
    if (!empty($term)) {
      $query = \Drupal::entityQuery('node');
      $query
      //->condition('status', 1)
      ...
      return '<speak>Die Methode "' . $title . '" wird beschrieben als: <break strength="x-strong" />' . $body . '</speak>';
    }
    else {
      return '<speak>Der Begriff, den du erklärt haben möchtest, kenne ich noch nicht.</speak>';
    }
  ...
  public function getMethodInfo($term_name = '', $term_info = '') {
    if (!empty($term_name) && !empty($term_info)) {
      $query = \Drupal::entityQuery('node');
      $query
      ->condition('type', 'Methode')
      ...
      foreach ($entity_ids as $nid) {
        $node = Node::load($nid);
        $title = $node->getTitle();
        $info = strip_tags($node->get('field_'. $term_info)->value, '<p></p>');
      }

      if (empty($info)) {
        $info = 'Es ist keine Information hierzu vorhanden.';
      }

      return '<speak>' . $info . '</speak>';
    }
    else {
      return '<speak>Die angeforderte Information kann ich nicht finden.</speak>';
    }
  }
